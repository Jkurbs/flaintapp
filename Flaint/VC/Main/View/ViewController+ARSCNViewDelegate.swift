...
extension ViewVC: ARSCNViewDelegate, ARSessionDelegate {
    ...
    func renderer(_ renderer: SCNSceneRenderer, didAdd node: SCNNode, for anchor: ARAnchor) {
        ...
        print("Did add object")
        
        DispatchQueue.main.async {
            self.statusViewController.cancelScheduledMessage(for: .planeEstimation)
            self.statusViewController.showMessage("SURFACE DETECTED")
            if self.virtualObjectLoader.loadedObjects.isEmpty {
                self.statusViewController.scheduleMessage("TAP TO PLACE ART", inSeconds: 7.5, messageType: .contentPlacement)
            }
        }
    }
...
//                material.diffuse.contents = UIImage(named: "test")
