...
class GallerySection: ListSectionController, ListAdapterDataSource, UIScrollViewDelegate,  UISearchBarDelegate, SearchSectionControllerDelegate {
    ...
    override init() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(self.slide), name: NSNotification.Name(rawValue: "slide"), object: nil)
    }
    ...
    @objc func slide(_ notification: NSNotification) {
        ...
        if let dict = notification.userInfo as NSDictionary? {
            if let direction = dict["direction"] as? String {
                if direction == "Right" {
                    ...
                    if currentIndex == self.arts.count - 1 {
            ...
            let count = ["index": currentIndex, "count": self.arts.count, "art": art] as [String : Any]
            NotificationCenter.default.post(name: Notification.Name("count"), object: nil, userInfo: count)
        }
    }
